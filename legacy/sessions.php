<?php
	...
	*/
	if(isset($_COOKIE['rr_sess'])){
		...
		$qry_mark = mysqli_query($sql_mark);
		$res_mark = mysqli_fetch_array($qry_mark);
	...
	if(isset($_POST['rr_selected'])){
		...
		if($_POST['rr_selected'] == 9999){
			...
			if($_POST['p_word'] == $ir['value']){
				...
				mysqli_query("UPDATE `ichange_parameters` SET `value` = '".$new_pw."' WHERE `param_name` = 'new_user_pass'");
				mysqli_query("UPDATE `ichange_parameters` SET `value` = '".$nupd."' WHERE `param_name` = 'new_user_pass_date'");
