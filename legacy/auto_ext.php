...
error_reporting(E_ALL); ini_set('display_errors', '1');
	...
	if($rec <= $rws && $create_po == 1){
		...
		for($i=0;$i<$no_2_gen;$i++){	
			...
			if(isset($rw['rr_id_from'])){
				$create_as = "P_ORDER"; if(strlen($rw['create_as']) > 0){ $create_as = $rw['create_as']; }
				...
				$email = "MRICC@groups.io"; //"MRICC@yahoogroups.com";
				//$email = "james@stanfordhosting.net";
				$headers = "From: mricf@stanfordhosting.net";
	...
	while($resr = $qwr->fetch_assoc()){
		$create_as = "WAYBILL"; if(strlen($resr['create_as']) > 0){ $create_as = $resr['create_as']; }
		$rws = $resr['id'];
