...
?>
	...
	function updateOnStatChg(stat,prog,locn,ulLab){
		...
		if(prog.value.length == 0){
			if(stat.value == 'CLOSED'){prog.value = 'WAYBILL CLOSED';}
	...
	<?php 
		...
		for($p=0;$p<count($prog_lst);$p++){
			...
			for($mi=0;$mi<count($map_json);$mi++){
				...
				$map_tmp = explode("(",$map_tmp);
				$map_tmp[0] = strtoupper($map_tmp[0]);
				if(strlen($map_tmp[0]) > 0){$map_arr[$map_tmp[0]] = 1;}	
			}
		}
	...
	// Auto complete calls
	$(function(){
		$("#fld6").autocomplete({ lookup: routes });
		$("#fld11").autocomplete({ lookup: commodities });
		$("#fld21_car").autocomplete({
			lookup: cars,
			onSelect: function (data) {
				$("#fld21_car").val(data.carnum);
				$("#fld21_aar").html('<option value="'+data.aartype+'">'+data.aartype+'</option>');
			}
		});
		$("#pfld6").autocomplete({	lookup: locations });
	});
