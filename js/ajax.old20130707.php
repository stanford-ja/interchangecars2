...
if(isset($_GET['f'])){
	...
	if($_GET['f'] == "selRoute"){selRoute(@$_GET['d'],@$_GET['s'],@$_GET['e']);}//,@$_GET['r']);}
...
function selTrain($fld14){
	...
	while($res = mysql_fetch_array($qry)){
		...
		if(strlen($res['auto']) > 4){
			...
			$lst .= "Exit Waypoint: <select id=\"exit_waypoint\" name=\"exit_waypoint\" onchange=\"route_valid8();\"><option value=\"\" selected=\"selected\">Select Exit</option>".$opts."</select>";
