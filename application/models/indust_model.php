<?php
class Indust_model extends CI_Model {
    ...
    function get_allSorted($fld='aar_code'){
        $query = $this->db->query("SELECT * FROM `".$this->tbl."` ORDER BY `".$fld."`");
        return $query->result();
    }

    function get_all4RR_Sorted($rr=0,$sort='indust_name'){
        $query = $this->db->query("SELECT * FROM `".$this->tbl."` WHERE `rr` = '".$rr."' OR `rr` = '0' ORDER BY `".$sort."`");
        return $query->result();
    }
