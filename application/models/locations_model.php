...
class Locations_model extends CI_Model {
    ...
    function get_allSorted($fld='fictional_location'){
        $query = $this->db->query("SELECT * FROM `".$this->tbl."` ORDER BY `".$fld."`");
        return $query->result();
    }

    function get_distinct($fld='fictional_location'){
        $query = $this->db->query("SELECT DISTINCT `".$fld."` FROM `".$this->tbl."` ORDER BY `".$fld."`");
        return $query->result();
    }
