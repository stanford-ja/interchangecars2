...
class Save_waybill extends CI_Controller {
	...
	function var_build(){
		...
		if($d_qry != 0 || strlen($d_qry) > 4){ // || count($d_wps) > 0){
			...
			$autoSav = array("autotrain" => 1, "route" => @json_decode($this->arr['route_json'],TRUE), "train_id" => $this->arr['fld14'][0], "waybill_num" => $this->arr['fld8'], "waybill_date" => $this->arr['fld1']);
		...
		//if($d_qry == 0 && strlen($this->arr['pfld3'][0]) > 0){
			...
			for($pz=0;$pz<count($this->arr['pfld4']);$pz++){
				...
				if($this->arr['pfld2'][$pz] == date('Y-m-d')){$pfld7_tmp = date('H:i');}
				$prog[] = array(
