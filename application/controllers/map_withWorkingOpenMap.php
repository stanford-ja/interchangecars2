	...
	*/
	...
	function wbCollector(){
		...
		*/
		...
		//$this->load->view("map_openMap",$this);
	...
	function openMap(){
		...
		for($i=0;$i<count($wb);$i++){
			...
			if(strlen($map_arr[0]) > 0 && isset($map_arr[1])){
				...
				$content['locations'][$map_arr[1].",".$map_arr[2]][] = $map_arr[0].",".$map_arr[1].",".$map_arr[2];
				$content['waybills'][$map_arr[1].",".$map_arr[2]][] = array('id' => $wb[$i]->id, 'wb_num' => $wb[$i]->waybill_num);
			}
		}
		//echo "<pre>";
		//print_r($content);
