...
class Save_waybill extends CI_Controller {
	...
	function var_build(){
		...
		$d_qry = $this->mricf->qry("ichange_trains", $this->arr['fld14'], "train_id", "auto");
		$d_wps = json_decode($d_qry, true);
		$fld14b = $this->arr['fld14'];
		if(strlen($this->arr['pfld3']) > 0){
		...
		if($d_qry != 0 || strlen($d_qry) > 4){ // || count($d_wps) > 0){
			//$autoSav = array("autotrain" => 1, "entry_waypoint" => $this->arr['entry_waypoint'], "exit_waypoint" => $this->arr['exit_waypoint'], "train_id" => $this->arr['fld14'], "waybill_num" => $this->arr['fld8'], "waybill_date" => $this->arr['fld1']);
			$autoSav = array("autotrain" => 1, "route" => @json_decode($this->arr['route_json'],TRUE), "train_id" => $this->arr['fld14'], "waybill_num" => $this->arr['fld8'], "waybill_date" => $this->arr['fld1']);
			$this->mricf->autoSav($autoSav); // insert record into ichange_auto, update ichange_waybill.progress.				//if($action != "NEW"){$this->arr['fld14'] = "AUTO TRAIN";}
			if($this->arr['id'] > 0){$this->arr['fld14'] = "AUTO TRAIN";}
		}elseif($this->arr['unload_days'] > 0){
			if(strlen($this->arr['pfld6']) < 1){$this->arr['pfld6'] = $prog[$last_prog]['map_locaton'];}
			$nxt_date = date('U') + (60*60*24*$this->arr['unload_days']);
			$nxt_date = date('Y-m-d',$nxt_date);
			$description = "UNLOADED";
			$autoSav = array("unload" => 1, "waybill_date" => $nxt_date, "waybill_num" => $this->arr['pfld4'], "description" => $description, "exit_waypoint" => $this->arr['pfld6']);
			$this->mricf->autoSav($autoSav); // insert record into ichange_auto, update ichange_waybill.progress.
			//$this->arr['pfld3'] = $this->arr['fld7']." *AUTOMATIC UNLOADING WILL BE COMPLETE ON ".$nxt_date."*";
			$this->arr['pfld3'] .= " *AUTOMATIC UNLOADING WILL BE COMPLETE ON ".$nxt_date."*";
		}	

		$this->arr['pfld6'] = str_replace(";",",",$this->arr['pfld6']);
		$this->arr['pfld6'] = str_replace(", ",",",$this->arr['pfld6']);
		$this->arr['pfld6'] = str_replace("  "," ",$this->arr['pfld6']);
		$this->arr['pfld6'] = trim($this->arr['pfld6']);

		// build progress report if not allocated to an AUTO TRAIN.
		$this->prog_sql = "";
		if($d_qry < 1 && strlen($this->arr['pfld3']) > 0){
		//if($d_qry == 0 && strlen($this->arr['pfld3']) > 0){
			$this->email_wb = 1;
			...
			$prog[] = array(
			'date' => $this->arr['pfld2'], 
			'time' => $this->arr['pfld7'].":".$this->arr['pfld8'], 
			'text' => strtoupper($this->arr['pfld3']),
			'waybill_num' => $this->arr['pfld4'], 
			'map_location' => strtoupper($this->arr['pfld6']), 
			'status' => strtoupper($this->arr['fld7']), 
		...
		if($d_qry < 1 && strlen($this->arr['pfld3']) > 0){$this->prog_sql = ", `progress` = '".$jprog."'";}
		...
		// Update car location.
		if(strlen($this->arr['pfld6']) > 0){
			for($cn=0;$cn<count($cars);$cn++){
				$carArrTmp = array('rr' => array($cars[$cn]['RR']), 'cars' => array($cars[$cn]['NUM']), 'lading' => $this->arr['fld11'], 'location' => strtoupper($this->arr['pfld6']));
	...
	function qry_build(){
		...
		else{$this->sql = "INSERT INTO `".$this->arr['tbl']."` SET ";}
		...
		$this->sql .= "`date` = '".strtoupper($this->arr['fld1'])."', 
			...
			`train_id` = '".strtoupper($this->arr['fld14'])."', 
	...
	function email_wb_to_grp(){
		...
		$message .= "Status: ".$this->arr['fld7']."\n";
		$message .= "Lading: ".$this->arr['fld11']."\n";
		$message .= "In Train: ".$this->arr['fld14']."\n";
