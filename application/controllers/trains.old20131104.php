...
class Trains extends CI_Controller {
	...
	public function lst(){
		...
		//$this->dat = array();
		$this->dat['fields'] 			= array('id', 'train_id', 'train_desc', 'loco_num', 'origin', 'destination', 'railroad_id', 'tr_sheet_ord','modified');
		$this->dat['field_names'] 		= array("ID", "Train ID", "Description", "Motive Power", "Origin", "Destination", "Railroad", "Sheet Order","Added/Modified");
		...
		$this->dat['links']				= array(
				'New' => "trains/edit/0",
				'TSheet [Sun]' => "trains/sheet/sun", 
				'TSheet [Mon]' => "trains/sheet/mon", 
				'TSheet [Tue]' => "trains/sheet/tues", 
				'TSheet [Wed]' => "trains/sheet/wed", 
				'TSheet [Thu]' => "trains/sheet/thu", 
				'TSheet [Fri]' => "trains/sheet/fri", 
				'TSheet [Sat]' => "trains/sheet/sat", 
	...
	function sheet($day="sun",$auto=0){
		...
		//$this->dat = array();
		$this->dat['fields'] 			= array('id', 'train_id', 'train_desc', 'no_cars', 'op_notes', 'direction', 'tr_sheet_ord', 'origin', 'destination');
		$this->dat['field_names'] 		= array("ID", "Train ID", "Description", "Max Cars", "Op Notes", "Direction", "Sheet Order", "Origin", "Destination");
		$this->dat['options']			= array(
	...
	public function setFieldSpecs(){
		...
		*/
		...
		$tr_tmp = (array)$this->Locomotives_model->getLocos4RR($this->arr['rr_sess']);
		for($i=0;$i<count($tr_tmp);$i++){$tr_opts[$tr_tmp[$i]->loco_num] = $tr_tmp[$i]->loco_num." - ".$tr_tmp[$i]->model;}
...
*/
		...
		$this->field_defs[] =  array(
			...
			'value' => '<div style="background-color: antiquewhite; border: 1px solid red; padding: 5px; font-size: 9pt;">
				<strong>Auto Train Waypoints</strong> <a href="javascript:{}" onclick="winOpn(\'../../../legacy/train_wps.php?id=\'+document.form.id.value,450,450);">Manage Auto Train Waypoints</a><br />
