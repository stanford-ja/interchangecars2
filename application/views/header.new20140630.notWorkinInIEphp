...
<html xmlns:fb="http://ogp.me/ns/fb#" itemscope itemtype="http://schema.org/Article">
	...
	//$(function(){
	var routes = [<?php 
	if(isset($this->Generic_model)){
		$fart = (array)$this->Generic_model->qry("SELECT `routing` FROM `ichange_waybill` ORDER BY `routing`");
		for($farty=0;$farty<count($fart);$farty++){echo "{ value: '".$fart[$farty]->routing."'},";}
	}
	?>];

	var commodities = [<?php 
	if(isset($this->Generic_model)){
		$fart = (array)$this->Generic_model->qry("SELECT `commod_name` FROM `ichange_commod` ORDER BY `commod_name`");
		for($farty=0;$farty<count($fart);$farty++){echo "{ value: '".$fart[$farty]->commod_name."'},";}
	}
	?>];

	var cars = [<?php 
	if(isset($this->Generic_model)){
		$fart = (array)$this->Generic_model->qry("SELECT CONCAT(`car_num`,',(',`aar_type`,'),',`desc`) AS `val`,`car_num`,`aar_type`,`desc` FROM `ichange_cars` WHERE `rr` = '".@$_COOKIE['rr_sess']."' ORDER BY `car_num`");
		for($farty=0;$farty<count($fart);$farty++){echo "{ value: '".$fart[$farty]->val."', carnum: '".$fart[$farty]->car_num."', aartype: '".$fart[$farty]->aar_type."', desc: '".$fart[$farty]->desc."'},";}
	}
	?>];
	//});
